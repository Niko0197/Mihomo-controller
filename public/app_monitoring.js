...
async function loadProxiesDashboard() {
  try {
    const [proxiesRes, providersRes, matchRes] = await Promise.all([
      fetch('/api/xkeen/proxies'),
      fetch('/api/xkeen/providers'),
      fetch('/api/config/match-target').catch(() => null)
    ]);
    ...
    if (matchRes && matchRes.ok) {
      try {
        ...
        if (matchData && matchData.target) currentMatchTarget = matchData.target;
        if (matchData && Array.isArray(matchData.options)) matchTargetOptions = matchData.options;
      } catch (e) {}
    }
...
function renderProxyGroups(proxiesData) {
  ...
  const excludeNames = ['GLOBAL', 'DIRECT', 'REJECT', '18+', 'MATCH'];
  const groups = [];
  ...
  // Добавляем единую карточку MATCH (переключатель финального правила - MATCH,<цель>)
  let availableMatchOptions = [];
  if (matchTargetOptions && Array.isArray(matchTargetOptions) && matchTargetOptions.length > 0) {
    availableMatchOptions = matchTargetOptions.filter(opt => (proxies[opt] || opt === 'DIRECT' || opt === 'REJECT') && opt !== 'MATCH' && opt !== 'GLOBAL');
  }
  if (availableMatchOptions.length === 0) {
    const defaultCandidates = ['🚀Auto-Best', '⚡ StealthSurf2', '⚙️Manual 1', '⚙️Manual 2', '⚙️Manual 3', 'DIRECT', 'REJECT'];
    availableMatchOptions = defaultCandidates.filter(opt => (proxies[opt] || opt === 'DIRECT' || opt === 'REJECT') && opt !== 'MATCH' && opt !== 'GLOBAL');
  }
  availableMatchOptions = Array.from(new Set(availableMatchOptions));

  const matchNow = currentMatchTarget || (proxies['GLOBAL'] && proxies['GLOBAL'].now) || 'DIRECT';

  const matchGroup = {
    name: 'MATCH',
    type: 'selector',
    icon: 'https://cdn.jsdelivr.net/gh/Koolson/Qure@master/IconSet/Color/Final.png',
    all: availableMatchOptions.length > 0 ? availableMatchOptions : ['🚀Auto-Best', 'DIRECT', 'REJECT'],
    now: matchNow,
    isMatchGroup: true
  };
  ...
  groups.sort((a, b) => {
    // MATCH всегда в самом конце панели
    if (a.name === 'MATCH') return 1;
    if (b.name === 'MATCH') return -1;
  ...
  groups.forEach(group => {
    ...
    const isCollapsed = localVal !== null ? localVal === 'true' : (group.isMatchGroup ? false : true);
    if (isCollapsed) {
    ...
    const resolvedActiveDelay = group.isMatchGroup 
      ? resolveSelectedProxyDelay(nowName, proxies) 
      : resolveSelectedProxyDelay(group.name, proxies);
    const nowDelay = resolvedActiveDelay;
    const delayText = resolvedActiveDelay > 0 ? `${resolvedActiveDelay} ms` : (nowName === 'DIRECT' ? 'DIRECT' : (nowName === 'REJECT' ? 'REJECT' : '—'));
    ...
    card.addEventListener('contextmenu', (e) => {
      ...
      e.preventDefault();
      const targetName = group.isMatchGroup ? (group.now || '🚀Auto-Best') : (group.name || group.now || '—');
...
async function selectMatchTarget(target) {
  currentMatchTarget = target;

  // 1. Оптимистичное обновление элементов карточки MATCH в DOM (0ms UI latency)
  try {
    const cards = document.querySelectorAll('.pgc-card');
    cards.forEach(card => {
      if (card.dataset.groupName === 'MATCH') {
        const selName = card.querySelector('.pgc-sel-name');
        if (selName) selName.textContent = target;

        const badge = card.querySelector('.pgc-count-badge');
        if (badge) {
          const d = resolveSelectedProxyDelay(target, proxyDashboardData?.proxies?.proxies || {});
          badge.textContent = d > 0 ? `${d} ms` : (target === 'DIRECT' ? 'DIRECT' : (target === 'REJECT' ? 'REJECT' : '—'));
          badge.style.color = getLatencyColor(d);
          badge.style.background = getLatencyBgColor(d);
        }
        ...
        btns.forEach(btn => {
          const nameSpan = btn.querySelector('.pgc-nb-name');
          if (nameSpan && nameSpan.textContent === target) {
            btn.classList.add('active');
          } else {
            btn.classList.remove('active');
          }
        ...
        const dots = card.querySelectorAll('.pgc-dot');
        dots.forEach(dot => {
          if (dot.title && dot.title.startsWith(target + ':')) {
            dot.classList.add('pgc-dot-active');
          } else {
            dot.classList.remove('pgc-dot-active');
          }
  ...
  // 2. Отправка команды на сервер для смены финального правила MATCH в config.yaml
  try {
    const res = await fetch('/api/config/match-target', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ target })
    });
    if (res.ok) {
      showToast('✅ MATCH → ' + target);
      loadProxiesDashboard().catch(() => {});
    } else {
      const err = await res.json().catch(() => ({}));
      showToast('Ошибка смены MATCH: ' + (err.error || res.statusText), 'error');
    }
  } catch (err) {
    showToast('Ошибка сети: ' + err.message, 'error');
  }
}
window.selectMatchTarget = selectMatchTarget;
