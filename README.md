...
- **Два независимых слота**:
  - `Слот 1: ⚡ ByeDPI 1 (ТВ)` *(порт 10805, служба `S52ciadpi-1`)*
  - `Слот 2: ⚡ ByeDPI 2 (Смартфон/ПК)` *(порт 10806, служба `S53ciadpi-2`)*
...
### 8. 🛡️ Настройка DNS и Резолвера (DNS Settings)

Полноценный центр управления встроенным DNS-сервером ядра Mihomo с защитой от утечек и подмены DNS-ответов.

<p align="center">
  <img src="screenshots/dns_settings.png" alt="Настройка DNS и Резолвера" width="100%">
</p>

- **Интерактивное переключение DoH/DoT пресетов**:
  - Быстрый выбор проверенных апстримов: `🛡️ AdGuard DNS (DoH + AdBlock)`, `⚡ Cloudflare (1.1.1.1 DoH)`, `🔒 Quad9 (DoH)`, `🌿 OpenDNS`, `🌐 Google (8.8.8.8)`, `🇷🇺 Yandex DNS (RU)` и `🏠 Локальный (127.0.0.1)`.
  - **Кнопка «✏️ Свой вариант»**: мгновенная очистка поля и перевод в режим кастомных адресов; при ручном вводе система сама распознаёт изменения и переключает активный статус.
- **Резервный DNS (Fallback) с GeoIP-фильтрацией**:
  - Защита от загрязнения DNS (DNS poisoning) и сбоев основных серверов.
  - Настройка кода страны (`RU` и др.) и фильтрации подсетей `ipcidr`.
- **Тонкие параметры ядра**:
  - Выбор режима резолва: `redir-host` (максимальная совместимость с ТВ и умным домом) или `fake-ip` (виртуальные IP).
  - Настройка адреса/порта прослушивания (`listen: 127.0.0.1:1053`) и переключатель резолва IPv6.
- **Горячее применение**: мгновенная запись в `config.yaml` и бесшовная перезагрузка DNS в ядре без разрыва активных соединений.

---

### 9. 📜 Системные логи ядра (Logs)
...
### 10. ⚙️ Системный центр и Управление ядром (System Hub)
...
Чтобы обновить установленную панель, выполните в SSH:
...
- 🛡️ **Полноценный модуль управления DNS и Резолвером**:
  ...
  - Пресет «✏️ Свой вариант» с автоматическим распознаванием любого ручного ввода, подсветкой и очисткой в 1 клик.
  - GeoIP-фильтрация Fallback DNS (защита от подмены и загрязнения ответов).
  - Управление режимами `enhanced-mode` (`redir-host` / `fake-ip`), адресом прослушивания `listen` и переключателем IPv6.
- 💻 **Улучшенный менеджер устройств сети**:
  - Монолитная табличная сетка: зафиксированы высоты строк и ячеек, устранены любые дергания и сдвиги элементов при изменении скоростей в реальном времени.
  - Умные выпадающие списки выбора VPN-маршрута и профилей ByeDPI/Zapret с плавной зеркальной анимацией и адаптивным определением направления раскрытия (вверх/вниз).
  - Ликвидация паразитных внутренних полос прокрутки (Zero Layout Shift).
- 🔒 **Безопасность и конфигурация**:
  - Исключение `config.yaml` из Git-трекинга и добавление чистого шаблона `config.example.yaml` для безопасного развёртывания без риска утечки приватных ссылок подписок.
  - Автоматическое определение активного системного демона роутера при перезапуске через веб-интерфейс (`S99vpn-updater-web` / `S99mihomo-controller`).
