...
function readRuleProviderLines(filePath) {
  ...
  try {
    ...
    for (const l of lines) {
      ...
      if (trimmed.startsWith('payload:')) { inPayload = true; continue; }
      if (inPayload) {
        if (trimmed.startsWith('-')) {
          let r = trimmed.substring(1).trim().replace(/^['"]|['"]$/g, '');
          if (r) rules.push(r);
...
function writeRuleProviderLines(filePath, rules) {
  const dir = path.dirname(filePath);
  if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
  const unique = [...new Set(rules)];
  let content = '# Generated by Mihomo Controller\npayload:\n';
  if (unique.length === 0) {
    content += '  # No rules\n';
  } else {
...
async function setClientRulesInConfig(ipsInput, targetGroup) {
  ...
  const providerNames = ['custom_autobest', 'custom_manual_1', 'custom_manual_2', 'custom_manual_3', 'custom_direct', 'custom_18_plus'];
  const providerMap = {
  ...
  const modifiedProviders = new Set();
  const rulesDir = '/opt/etc/mihomo/rules';

  for (const provName of providerNames) {
    const provPath = path.join(rulesDir, `${provName}.yaml`);
    let rules = readRuleProviderLines(provPath);
    const originalLen = rules.length;
    
    rules = rules.filter(r => {
      for (const targetIp of ips) {
        if (r.startsWith(`SRC-IP-CIDR,${targetIp}/32`) || r.startsWith(`SRC-IP-CIDR,${targetIp}/128`)) {
          return false;
        }
      }
      return true;
    ...
    const targetProvName = providerMap[targetGroup];
    if (targetProvName === provName) {
      for (const targetIp of ips) {
        const isIpv6 = targetIp.includes(':');
        const mask = isIpv6 ? '/128' : '/32';
        rules.push(`SRC-IP-CIDR,${targetIp}${mask}`);
      }
    }

    if (rules.length !== originalLen || targetProvName === provName) {
      writeRuleProviderLines(provPath, rules);
      modifiedProviders.add(provName);
    }
  }
